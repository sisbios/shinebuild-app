...
export async function generateStaffQrTokenAction(): Promise<GenerateQrResult> {
  const session = await getServerSession();
  if (!session || session.role !== 'staff' || session.status !== 'approved') {
  ...
  try {
    ...
    const tokenRef = await db.collection(COLLECTIONS.QR_TOKENS).add({
      agentId: session.uid, // staff uid — lead will be attributed to this staff member
      tokenHash,
    ...
    console.error('generateStaffQrTokenAction error:', err);
