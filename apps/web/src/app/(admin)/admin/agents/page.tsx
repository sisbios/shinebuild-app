...
export default async function AdminAgentsPage({ searchParams }: Props) {
  ...
  try {
    ...
    const counts = await Promise.all(
      raw.map((a: any) =>
        db.collection(COLLECTIONS.LEADS).where('agentId', '==', a.uid).get()
  ...
  const counts = { pending: 0, approved: 0, rejected: 0, deactivated: 0 };
  agents.forEach((a) => { if (a.status in counts) counts[a.status as keyof typeof counts]++; });
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        {(['', 'pending', 'approved', 'rejected', 'deactivated'] as const).map((s) => (
          <Link
            key={s}
            href={s ? `/admin/agents?status=${s}` : '/admin/agents'}
          ...
          >
            {s || 'All'} {s && `(${counts[s as keyof typeof counts]})`}
          </Link>
        ))}
        <span className="ml-auto flex gap-2">
          <Link href={`/admin/agents?${sp.status ? `status=${sp.status}&` : ''}sort=date`} className="rounded-full px-3 py-1.5 text-xs font-semibold glass text-gray-600 hover:text-gray-900">
            Sort: Date
          </Link>
          <Link href={`/admin/agents?${sp.status ? `status=${sp.status}&` : ''}sort=leads`} className="rounded-full px-3 py-1.5 text-xs font-semibold glass text-gray-600 hover:text-gray-900">
            Sort: Leads
          </Link>
          <Link href={`/admin/agents?${sp.status ? `status=${sp.status}&` : ''}sort=name`} className="rounded-full px-3 py-1.5 text-xs font-semibold glass text-gray-600 hover:text-gray-900">
            Sort: Name
          </Link>
        </span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/40">
                ...
                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400 hidden sm:table-cell">Phone</th>
                ...
                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400 hidden lg:table-cell">Leads</th>
            ...
            <tbody className="divide-y divide-white/30">
              {agents.map((agent) => (
                <tr key={agent.uid} className="hover:bg-white/30 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-gray-600 hidden sm:table-cell">{agent.phone}</td>
                  ...
                  <td className="px-5 py-3.5 text-gray-700 font-medium hidden lg:table-cell">{agent.leadCount}</td>
