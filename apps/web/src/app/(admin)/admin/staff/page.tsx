...
import { SuperAdminStaffActions } from './SuperAdminStaffActions';
...
export default async function AdminStaffPage() {
  ...
  try {
    // Superadmin sees staff + admin + superadmin; regular admin sees only staff
    const snap = isSuperAdmin
      ? await db.collection(COLLECTIONS.USERS).where('role', 'in', ['staff', 'admin', 'superadmin']).get()
      : await db.collection(COLLECTIONS.USERS).where('role', '==', 'staff').get();
    const members = snap.docs
      .filter((doc) => doc.id !== session?.uid) // never show yourself
      .map((doc) => {
        const d = doc.data();
        return { uid: doc.id, name: d['name'] ?? '—', phone: d['phone'] ?? '—', role: d['role'] ?? 'staff', status: d['status'] ?? 'active', createdAt: d['createdAt']?.toDate() ?? new Date(), leadCount: 0, assignedForAssessment: d['assignedForAssessment'] ?? false };
      });
          ...
          <div key={s.uid} className="glass-card rounded-2xl p-4">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {s.assignedForAssessment && (
                  <span className="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">QA</span>
                )}
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              {isSuperAdmin ? (
                <SuperAdminStaffActions
                  uid={s.uid} name={s.name} phone={s.phone} role={s.role}
                  leadCount={s.leadCount}
                  otherStaff={staff.filter((o) => o.uid !== s.uid && o.status === 'active')}
                />
              ) : (
                <StaffActions uid={s.uid} name={s.name} currentStatus={s.status} assignedForAssessment={s.assignedForAssessment} canPromote={false} />
              )}
            </div>
          </div>
        ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/40">
                ...
                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400 hidden md:table-cell">QA</th>
            ...
            <tbody className="divide-y divide-white/30">
              {staff.map((s) => (
                <tr key={s.uid} className="hover:bg-white/30 transition-colors">
                  ...
                  <td className="px-5 py-3.5 hidden md:table-cell">
                    {s.assignedForAssessment ? (
                      <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200">Assigned</span>
                    ) : (
              ...
              {staff.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-gray-400 text-sm">No staff members yet. Add one above.</td></tr>
              )}
            </tbody>
          </table>
        </div>
